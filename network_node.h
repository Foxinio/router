...
public:
    ...
    network_node(uint32_t network_ip, uint8_t mask, uint32_t route_addr, uint32_t route_mask, uint32_t dist);
...
public:
    int score(const interface& i);
    best_fit(uint32_t ip);
    bool operator()(const interface &l, const interface &r);
